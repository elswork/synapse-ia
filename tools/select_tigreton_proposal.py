...
class TigretonSelector:
    def __init__(self, base_path=None):
        self.base_path = base_path or os.environ.get("BASE_PATH", "/app")
        self.registry_path = os.path.join(self.base_path, "context/data/tigreton_registry.json")
    ...
    def generate_proposal(self, name_filter=None):
        ...
        if not selection:
            return {"error": "No hay contactos de poder (Tigretones) disponibles."}
        ...
        Rol/Posición: {expert_data.get('role', 'Líder Estratégico')}
        ...
        ESTRUCTURA SUGERIDA:
        1. Saludo directo.
        2. Mención a su posición en {expert_data.get('role', 'el ecosistema')}.
        ...
        try:
            ...
            email_data = {
                "subject_local": f"Alianza Estratégica Anticitera ({expert_data.get('country', '')})",
                "body_local": response_text,
                "subject_spanish": "Alianza Estratégica Anticitera",
...
---
*Generated by Athena for Operación Tigretón on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*
"""
